...
for a in ${po_file}
do
	grep -q 'Language: zh_CN' "$a" 2>/dev/null && sed -i 's/Language: zh_CN/Language: zh_Hans/g' "$a"
...
for b in ${po_file2}
do
	grep -q 'Language: zh_CN' "$b" 2>/dev/null && sed -i 's/Language: zh_CN/Language: zh_Hans/g' "$b"
